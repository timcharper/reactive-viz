...
//   implicit class EnrichedSource[Out, M1, M2](source: Source[Source[Out, M2], M1]) {
//     def mergeUnordered(implicit system: ActorSystem, materializer: Materializer): Source[Out, M1] =
//       apply(source)
//   }
...
//   def apply[K, M1, M2](streams: Source[Source[K, M2], M1], maxQueueLength: Int = 10)(implicit system: ActorSystem, materializer: Materializer): Source[K, M1] = {
//     def boot(publisher: ActorRef, publisherBlockingStrategy: RequestStrategy): Unit = {
//       import system.dispatcher
//       streams.
//         map { m =>
//           m.runWith(Sink.actorSubscriber(Props(new StreamSubscriber(publisher, publisherBlockingStrategy))))
//         }.
//         runFold(0) { (a, _) => a + 1 }.
//         onComplete {
//           case Success(n) =>
...
//     Source.actorPublisher[K](Props(new MergedStreamPublisher(maxQueueLength, boot)))
//   }
// }
